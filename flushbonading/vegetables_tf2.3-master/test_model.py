...
def test_mobilenet():
    ...
    train_ds, test_ds, class_names = data_load("D:/ali_download/flushbonading/vegetables_tf2.3-master/new_data/train",
                                              "D:/ali_download/flushbonading/vegetables_tf2.3-master/new_data/val", 224, 224, 16)
    # todo 加载模型，修改为你的模型名称
...
def test_cnn():
    ...
    train_ds, test_ds, class_names = data_load("D:/ali_download/flushbonading/vegetables_tf2.3-master/new_data/train",
                                              "D:/ali_download/flushbonading/vegetables_tf2.3-master/new_data/val", 224, 224, 16)
    # todo 加载模型，修改为你的模型名称
    model = tf.keras.models.load_model("models/cnn_peachs.h5")
    # model.summary()
    # 测试
    loss, accuracy = model.evaluate(test_ds)
    # 输出结果
